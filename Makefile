...
UBUNTU = $(shell cat /etc/*-release | grep -o -m 1 Ubuntu)
DEBIAN = $(shell cat /etc/*-release | grep -o -m 1 Debian)
FIND = find
TOUCH = -exec touch {} +
...
CFLAGS := -ansi -pedantic-errors -Wall -Werror -Wno-deprecated-declarations \
  $(CFLAGS)
CFLAGS/$(DBG) = -O0 -g -DJPC_DEBUG -fsanitize=address -fno-omit-frame-pointer
CFLAGS/$(OPT) = -O3

LDFLAGS := -Wall -lm -lpthread $(LDFLAGS)
...
DIST_DOCS = Makefile _LICENSE _README _VIM
...
$(BUILD)/$(1)/$(BMK)/$(2).mk: $(BUILD)/$(1)/$(OMK)/$(2).mk \
  | $(BUILD)/$(1)/$(BMK)/
	@set -e; echo "Make binary makefile $$@"; rm -f $$@; \
		sed '1s|$(OBJ)|$(BIN)| ; s|\.o|| ; s| [^ ]*\.c||g ; s|\.h|.o|g ; \
	...
	if grep -q $(TIFF_FILE) $(BUILD)/$(1)/$(OMK)/$(2).mk; then \
	  $(ECHO_E) "-ltiff \c" >> $$@; \
	fi; \
...
repos: $(REPOS_MK) $(foreach r,$(REPOS),$(REPO)/$(r))


.PHONY: default opt all check clean%  $(DTARGETS) $(OTARGETS) \
  d/all o/all a/all show/l show/p show/t test otest dist/% repo/% repos \
